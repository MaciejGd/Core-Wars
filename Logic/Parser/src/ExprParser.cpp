...
ParseResult CExprParser::EvaluateExpression(std::deque<Token>& tokens, int& result)
{
    ...
    result = m_EvaluateArithmeticExpression(expression_tokens);
    return ParseResult::PARSE_OK;
}

ParseResult CExprParser::m_ParseArithmeticExpression(std::deque<Token> &tokens)
{
...
int CExprParser::m_PrecedenceHigher(Token& first_sign, Token& second_sign)
{
...
void CExprParser::m_TranslateToPostfix(std::deque<Token>& input, std::deque<Token> &postfix)
{
    std::stack<Token> op;
...
int CExprParser::m_EvaluatePostifxExpression(std::deque<Token> &postfix)
{
...
int CExprParser::m_EvaluateArithmeticExpression(std::deque<Token>& tokens)
{   
